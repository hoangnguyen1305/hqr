part of '../list_plate_page.dart';

class ListPlate extends StatefulWidget {
  const ListPlate({super.key});

  @override
  State<ListPlate> createState() => _ListPlateState();
}

class _ListPlateState extends State<ListPlate> {
  ...
  void initState() {
    super.initState();
    _bloc = context.read<ListPlateBloc>();
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ListPlateBloc, ListPlateState>(
      buildWhen:
          (pre, cur) =>
          ...
          Row(
            children: [
              Expanded(
                child: CText(
                  text:
                      'Giá khởi điểm:\n${CurrencyUtils.format(plate.startingPrice)}',
                  maxLines: 10,
                ),
              ),
              const SizedBox(width: ConstRes.defaultHorizontal),
